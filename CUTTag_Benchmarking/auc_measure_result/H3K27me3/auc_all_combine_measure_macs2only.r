...
method <- c("macs2broad","macs2broadATAC","macs2broadIgG")
...
write.table(score_ROC,file=paste0(path2,"auROCscore_macs2only.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(score_PRC,file=paste0(path2,"auPRCscore_macs2only.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_ROC,file=paste0(path2,"auROCrank_macs2only.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_PRC,file=paste0(path2,"auPRCrank_macs2only.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_mean,file=paste0(path2,"AUCmeanrank_macs2only.txt"),row.names=T,col.names=T,sep="\t",quote=F)
