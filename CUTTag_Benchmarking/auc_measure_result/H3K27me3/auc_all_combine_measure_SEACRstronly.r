...
method <- c("SEACRstr","SEACRstrATAC","SEACRstrIgG")
...
write.table(score_ROC,file=paste0(path2,"auROCscore_SEACRstronly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(score_PRC,file=paste0(path2,"auPRCscore_SEACRstronly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_ROC,file=paste0(path2,"auROCrank_SEACRstronly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_PRC,file=paste0(path2,"auPRCrank_SEACRstronly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_mean,file=paste0(path2,"AUCmeanrank_SEACRstronly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
