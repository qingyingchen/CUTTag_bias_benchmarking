...
method <- c("RSEGdz","RSEGdzATAC","RSEGdzIgG")
...
write.table(score_ROC,file=paste0(path2,"auROCscore_RSEGdzonly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(score_PRC,file=paste0(path2,"auPRCscore_RSEGdzonly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_ROC,file=paste0(path2,"auROCrank_RSEGdzonly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_PRC,file=paste0(path2,"auPRCrank_RSEGdzonly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_mean,file=paste0(path2,"AUCmeanrank_RSEGdzonly.txt"),row.names=T,col.names=T,sep="\t",quote=F)
