...
truedata <- read.table("/standard/vol190/zanglab/sh8tv/Project/CUTRUN/Result/K562_H3K27me3_H3K27ac/TrueFalseRegion/H3K27ac/H3K27ac_true_regions.bed")
true_total <- sum(truedata[,3]-truedata[,2])
falsedata <- read.table("/standard/vol190/zanglab/sh8tv/Project/CUTRUN/Result/K562_H3K27me3_H3K27ac/TrueFalseRegion/H3K27ac/H3K27ac_false_regions.bed")
false_total <-  sum(falsedata[,3]-falsedata[,2])

CUTTags = c("GSM3536514")

len_CUTTag <-length(CUTTags)
method <- c("macs2","macs2ATAC","macs2IgG","macs2broad","macs2broadATAC","macs2broadIgG","SEACRstr","SEACRstrATAC","SEACRstrIgG","SEACRrel","SEACRrelATAC","SEACRrelIgG","MUSIC","MUSICATAC","MUSICIgG","GoPeaks","GoPeaksATAC","GoPeaksIgG","RSEG","RSEGATAC","RSEGIgG","RSEGdz","RSEGdzATAC","RSEGdzIgG")
...
cutoffs<- cbind(cc1,cc1,cc1,a1,a1,a1,d1,f,f,ccc1,f,f,cc1,cc1,cc1,cc1,cc1,cc1,b1,b1,b1,b1,b1,b1)
...
for (i in (1:len_CUTTag)){
  ...
  for (j in (1:len_method)){
    ...
    write.table(outdata,paste0(path2,CUTTags[i],"_",method[j],"_measures_new.txt"),row.names = T, col.names = T,  sep=" ",quote=F)
    ROC<- outdata[,c(6,5)]
  ...
  score_mean <- apply(auc,1,mean)
  final_score_mean<-cbind(final_score_mean,score_mean)
...
colnames(final_score_mean) <- CUTTags
rownames(final_score_mean) <- method
write.table(score_ROC,file=paste0(path2,"auROCscore_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(score_PRC,file=paste0(path2,"auPRCscore_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_score_mean,file=paste0(path2,"AUCmeanscore_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_ROC,file=paste0(path2,"auROCrank_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_PRC,file=paste0(path2,"auPRCrank_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
write.table(final_mean,file=paste0(path2,"AUCmeanrank_ac_new.txt"),row.names=T,col.names=T,sep="\t",quote=F)
