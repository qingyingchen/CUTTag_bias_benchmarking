...
method <- c("macs2broadATAC","macs2broadIgG","MUSICATAC","MUSICIgG","GoPeaksATAC","GoPeaksIgG","RSEGATAC","RSEGIgG","RSEGdzATAC","RSEGdzIgG")
...
measure_data <- data.frame(matrix(nrow = 10, ncol = 6))
...
scoremean <- scoremean[c(2,3,11,12,14,15,17,18,20,21),]

auc <- read.table(paste0(path2,CUTTags[1],"_auc.txt"))
auc <- auc[c(2,3,11,12,14,15,17,18,20,21),]
...
write.table(measure_data,file=paste0(path2,"AUC_ac_ctrl.txt"),row.names=T,col.names=T,sep="\t",quote=F)
