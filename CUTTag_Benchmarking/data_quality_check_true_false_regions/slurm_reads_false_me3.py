...
for f in allf:
    ...
    if os.path.isfile(folder +f) and "H3K27me3" in f and "_CUTTag_" in f and f.endswith("_PE_sorted.bed"):
...
#SBATCH -p standard
#SBATCH -A cphg_cz3d
#SBATCH -o %s_reads_false.log
#SBATCH -e %s_reads_false.log

module load gcc
module load bedtools

cat %s_false.bed| awk 'BEGIN{OFS="\t";FS="\t"}{print $1,$2,$3}'| intersectBed -a - -b /standard/vol190/zanglab/shared/CUTTag/true_false/PEbed/%s_PE_sorted.bed -wao > %s_reads_false.bed

"""%(ID,ID,ID,ID,ID)
        outf = open ('%s_reads_false.slurm'%(ID),'w')
        outf.write(model)
        outf.close()
