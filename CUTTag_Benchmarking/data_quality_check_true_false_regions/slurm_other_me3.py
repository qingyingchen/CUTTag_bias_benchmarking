...
#SBATCH -p standard
#SBATCH -A cphg_cz3d
#SBATCH -o %s_other.log
#SBATCH -e %s_other.log

module load gcc
module load bedtools

cat /project/zanglab_project/jkx9nz/project/cut/FACTOR/HUMAN/combineReads/sicer/bed/acme3/%s_sicerPeaks_sorted.bed| awk 'BEGIN{OFS="\t";FS="\t"}{print $1,$2,$3+1,$4}'|intersectBed -a - -b /project/zanglab_project/jkx9nz/project/cut/FACTOR/HUMAN/combineReads/sicer/overlap/strict_false/merge_false200_sorted.bed -v | intersectBed -a - -b /project/zanglab_project/jkx9nz/project/cut/FACTOR/HUMAN/combineReads/sicer/overlap/strict_true/strong_true200_sorted.bed -v > %s_peaks_other.bed

"""%(ID,ID,ID,ID)
        outf = open ('%s_other.slurm'%(ID),'w')
        outf.write(model)
        outf.close()
