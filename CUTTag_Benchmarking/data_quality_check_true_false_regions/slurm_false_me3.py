...
#SBATCH -p standard
#SBATCH -A cphg_cz3d
#SBATCH -o %s_false.log
#SBATCH -e %s_false.log

module load gcc
module load bedtools

cat /project/zanglab_project/jkx9nz/project/cut/FACTOR/HUMAN/combineReads/sicer/bed/acme3/%s_sicerPeaks_sorted.bed | awk 'BEGIN{OFS="\t";FS="\t"}{print $1,$2,$3+1,$4}'|intersectBed -a - -b /project/zanglab_project/jkx9nz/project/cut/FACTOR/HUMAN/combineReads/sicer/overlap/strict_false/merge_false200_sorted.bed > %s_false.bed

"""%(ID,ID,ID,ID)
        outf = open ('%s_false.slurm'%(ID),'w')
        outf.write(model)
        outf.close()
