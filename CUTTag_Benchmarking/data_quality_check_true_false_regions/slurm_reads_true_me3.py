...
#SBATCH -p standard
#SBATCH -A cphg_cz3d
#SBATCH -o %s_reads_true.log
#SBATCH -e %s_reads_true.log

module load gcc
module load bedtools

cat %s_true.bed| awk 'BEGIN{OFS="\t";FS="\t"}{print $1,$2,$3}'| intersectBed -a - -b /standard/vol190/zanglab/shared/CUTTag/true_false/PEbed/%s_PE_sorted.bed -wao > %s_reads_true.bed

"""%(ID,ID,ID,ID,ID)
        outf = open ('%s_reads_true.slurm'%(ID),'w')
        outf.write(model)
        outf.close()
