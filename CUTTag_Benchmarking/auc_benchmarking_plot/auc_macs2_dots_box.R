...
auroc_rank <- read.table("/Users/jkx9nz/Library/CloudStorage/Dropbox/CUTTagResults/result/benchmarking/cutoff_all_sicer/measures/auROCrank_macs2only.txt",
                         header=T,sep="\t")
auprc_rank <- read.table("/Users/jkx9nz/Library/CloudStorage/Dropbox/CUTTagResults/result/benchmarking/cutoff_all_sicer/measures/auPRCrank_macs2only.txt",
                         header=T,sep="\t")
mean_rank <- read.table("/Users/jkx9nz/Library/CloudStorage/Dropbox/CUTTagResults/result/benchmarking/cutoff_all_sicer/measures/AUCmeanrank_macs2only.txt",
                        header=T,sep="\t")
...
colnames(auroc_rank) <- c("macs2","-ATAC","-IgG")
colnames(auprc_rank) <- c("macs2","-ATAC","-IgG")
colnames(mean_rank) <- c("macs2","-ATAC","-IgG")

pdf("/Users/jkx9nz/Library/CloudStorage/Dropbox/CUTTagResults/result/benchmarking/cutoff_all_sicer/figures_17me3/AUC_macs2only_dots_box.pdf",w = 6,h=4)
par(mfcol=c(1,3))
#par(mar=c(8,5,3,5))
...
plot(0, type="n",main="auROC Rank macs2", xlim=c(0.5,3.5), ylim=c(0,1), yaxt="n", xaxt="n", xlab="", ylab="",cex.axis=2)
...
plot(0, type="n",main="auPRC Rank macs2", xlim=c(0.5,3.5), ylim=c(0,1), yaxt="n", xaxt="n", xlab="", ylab="",cex.axis=2)
axis(2, at=seq(0,1,0.2), las=1, tck=-0.06,cex.axis=2)
box_pos <- c(1:3)
...
plot(0, type="n", main="AUCmean Rank macs2",xlim=c(0.5,3.5), ylim=c(0,1), yaxt="n", xaxt="n", xlab="", ylab="",cex.axis=2)
axis(2, at=seq(0,1,0.2), las=1, tck=-0.06,cex.axis=2)
box_pos <- c(1:3)
